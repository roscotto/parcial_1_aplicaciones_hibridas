...
function exportarHtml(proyectos) {
    let html = `
    <!DOCTYPE html>
<html lang="es">
...
<body>
        ...
        <div class="justify-content-end collapse navbar-collapse" id="navbarNavAltMarkup">
          <div class="navbar-nav">
            ...
            <div class="nav-link dropdown">
              <a id="linkProyectos" class="text-reset text-decoration-none dropdown-toggle" href="#" role="button"
  ...
  <main class="mt-5">
    ...
    for (let i = 0; i < proyectos.length; i++) {
        html += `
                <div>
                <h3>${proyectos[i].name}</h3>
                <p>${proyectos[i].description}</p>
                <p>${proyectos[i].technologies}</p>
                <a href="${proyectos[i].link}">Link</a>
                <p>${proyectos[i].section}</p>
                <p>${proyectos[i].img}</p>
                </div>`;
...
/* -------------------------------------------------------------------------------------------------------------*/
async function traerInfoProyectos(req, res) {
    const seccion = req.path.split("/")[1];
    console.log('[vistasProyectosController.js]', seccion)

    proyectosServices.proyectoPorSeccion({ "section": seccion })
        .then(function (proyectos) {
            res.send(exportarHtml(proyectos));
        })
