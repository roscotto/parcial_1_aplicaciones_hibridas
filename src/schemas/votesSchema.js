...
export const toVoteCreateSchema = yup.object(
    {
        ...
        arte_score: yup.number().integer().positive().min(1).max(10).required(),
        sonido_score: yup.number().integer().positive().min(1).max(10).required(),
        afinidad_score: yup.number().integer().positive().min(1).max(10).required()
    }
);
